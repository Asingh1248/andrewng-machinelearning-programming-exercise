...
%
 ...
 %%%%%%%%%%% Part 1: Calculating J(Cost Function ) w/o Regularization %%%%%%%%%%%%%%%
 ...
 % Adding 1 as first column in z = (Adding bias unit) % m x (hidden_layer_size + 1) == 5000 x 26
 a2 = [ones(size(a2,1),1),a2];  
 z3 = a2 * Theta2'; %  m x num_labels == 5000 x 10
 a3 = sigmoid(z3);  % m x num_labels == 5000 x 10
 
 h_x = a3 ; % m x num_labels == 5000 x 10 (hypothesis for output )
 
 % Converting y into vectors of 0's and 1's for multiclass classification
 ...
 %%%%%%%%%%% Part 2: Implementing Backpropogation for Theta1_grad and Theta2_grad. w/o Regularization %%%%%%%%%%%%%
 ...
 for t= 1:m
   ...
   % layer2
   z2= Theta1 * a1; %hidden_layer_size x 1 === 25x1
   a2=[1;sigmoid(z2)];% (hidden_layer_size+1) x 1 == 26 x 1
   
   % layer 3
   z3 = Theta2 * a2; %num_labels x 1 == 10 x1
   a3 = sigmoid(z3); %num_labels x 1 == 10 x 1
   yVector = (1:num_labels)'==y(t); % num_labels x 1 == 10 x 1 [Shortvut of for loop for y_vec]
   
   %Calculating delta values
   
   delta3 = a3 - yVector ; % num_labels x 1 == 10 x1
   
   delta2 = (Theta2' * delta3).*[1;sigmoidGradient(z2)] ; %(hidden_layer_size+1) x 1 == 26 x 1
   
   delta2 = delta2(2:end);  %Removing delta2 for bias node  %hidden_layer_size x 1 == 25 x 1
   
   % delta_1 is not calculated beacuse we do not associate error with input  
   Theta1_grad = Theta1_grad + (delta2 * a1'); %25 x 401
   Theta2_grad = Theta2_grad + (delta3 * a2'); %10 x 26 (3rd layer(No of unit) to 2nd layer(No of unit))
 ...
 reg_term  = reg_term = (lambda/(2*m)) * (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2))); %scalar
